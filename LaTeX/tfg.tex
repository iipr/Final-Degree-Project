...
Este trabajo contiene distintas pruebas del mismo hecho y pretende hacer una comparación computacional entre ellas, tratando de encontrar la m\'as eficiente.

Sea $R$ un cuerpo real cerrado y $n\geq 2$. Entonces: 
	
(1) Para cada subconjunto finito $F$ de $R^n$, el conjunto semialgebraico $R^n\setminus F$ es imagen polinomial de $R^n$. 
	
(2) Dadas unas formas linealmente independientes $h_1,\dots,h_r$ de $R^n$, el conjunto semialgebraico $\{h_1>0,\dots,h_r>0\}\subset R^n$ es imagen polinomial de $R^n$.
	
El resultado clave aqu\'i es que $\Qu:=\{x>0,y>0\}$ es imagen polinomial de $R^2$. Esta afirmaci\'on es probada de tres maneras diferentes: una primera usando algunos resultados b\'asicos de la geometr\'ia algebraica real (como son el teorema de Sturm y el Lema de Selecci\'on de Curvas) y la ayuda de un odenador; una segunda forma m\'as corta, usando la composici\'on de tres aplicaciones polinomiales bastante sencillas; y una tercera y \'ultima que comprende argumentos de la rama de la topolog\'ia algebraica, sin la necesidad de realizar computaciones en un ordenador. 
...
\draw (c2) node {$y$};
\end{tikzpicture}
\vspace{0.15cm}\caption{$\FF_1(x, y) = (xy - 1)^2 + x^2$.\label{fig:FF_1}}
\end{subfigure}
\begin{subfigure}{.49\linewidth}\centering
\begin{tikzpicture}
...
\draw (c2) node {$y$};
\end{tikzpicture}
\vspace{0.15cm}\caption{$\FF_2(x, y) = (xy - 1)^2 + y^2$.\label{fig:FF_2}}
\end{subfigure}\\[1ex]
\vspace{0.4cm}
...
\draw (c2) node {$y$};
\end{tikzpicture}
\vspace{0.15cm}\caption{$\GG_1(x, y) = x$.\label{fig:GG_1}}
\end{subfigure}
\begin{subfigure}{.49\linewidth}\centering
\begin{tikzpicture}
...
\draw (c2) node {$y$};
\end{tikzpicture}
\vspace{0.15cm}\caption{$\GG_2(x, y) = y(xy - 2)^2 + x (xy - 1)^2$.\label{fig:GG_2}}
\end{subfigure}\\[1ex]
\vspace{0.4cm}
...
\draw (c2) node {$y$};
\end{tikzpicture}
\vspace{0.15cm}\caption{$\HH_1(x, y) = x(xy - 2)^2 + \tfrac{1}{2}xy^2$.\label{fig:HH_1}}
\end{subfigure}
\begin{subfigure}{.49\linewidth}\centering
\begin{tikzpicture}
...
\begin{figure}[h]
\hspace{-1.9cm}
\begin{subfigure}{.73\linewidth}\centering
\begin{tikzpicture}
...
\begin{subfigure}{.73\linewidth}\centering
\begin{tikzpicture}
\node[anchor=south west,inner sep=0] (img)at (0,0) {\includegraphics[width=0.7\textwidth]{plots/ch2_09_B_vert.pdf}};
...
\path ($(img.north west)!0.25!(img.north east)$)|-coordinate(c8)($(img.south east)!0.55!(img.north east)$); % Para B_{x_0}
\path ($(img.north west)!0.76!(img.north east)$)|-coordinate(c9)($(img.south east)!0.35!(img.north east)$); % Para B_{x_1}
\draw (c1) node {\huge{$\mathscr{B}$}};
...
\begin{subfigure}{.73\linewidth}\centering
\begin{tikzpicture}
\node[anchor=south west,inner sep=0] (img)at (0,0) {\includegraphics[width=0.7\textwidth]{plots/ch2_10_B_hor.pdf}};
\path ($(img.north west)!0.6!(img.north east)$)|-coordinate(c1)($(img.south east)!0.7!(img.north east)$);% c1: lugar donde poner \mathscr{B}
\path ($(img.north west)!0.8!(img.north east)$)|-coordinate(c1)($(img.south east)!0.8!(img.north east)$); % Para \mathscr{B}
\path ($(img.north west)!0.9!(img.north east)$)|-coordinate(c2)($(img.south east)!0.604!(img.north east)$); % Para la flecha de y_0
\path ($(img.north west)!0.9!(img.north east)$)|-coordinate(c3)($(img.south east)!0.1495!(img.north east)$); % Para la flecha de y_1
...
\draw (c8) node {\large{\color[rgb]{0,0.392157,0}{$(\mathscr{B}_{y_0} \times \set{y_0})$}}};
\draw (c9) node {\large{\color[rgb]{1,0,0}{$(\mathscr{B}_{y_1} \times \set{y_1})$}}};
\draw (c1) node {\huge{$\mathscr{B}$}};
\end{tikzpicture}
\caption{$\mathscr{B} = \bigsqcup_{y>0}(\mathscr{B}_{y} \times \set{y})$.\label{fig:Bhor}}
\end{subfigure}
