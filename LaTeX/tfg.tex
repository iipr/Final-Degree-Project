...
\draw [line width=0.8pt, draw=red!60, <->] (0.05,1.45) arc (190:510:0.2cm);
...
As $\theta_1=\tfrac{\pi}{2}-\theta_2$, the map $\lambda:=\mu|_{\overline{\Rr}_M}:\overline{\Rr}_M\to\overline{\Tt}_M$ factors through $\tilde{\overline{\Rr}}_M$ and there exists a continuous map $\tilde{\lambda}:\tilde{\overline{\Rr}}_M\to\overline{\Tt}_M$ such that the following diagram is commutative.
\begin{center}
\begin{tikzpicture}[scale=1.5]
\node (A0) at (0,1) {$\Rr_M$};
\node (A) at (1,1) {$\overline{\Rr}_M$};
...
\end{tikzpicture}
\end{center}
The map $\tilde{\lambda}$ is continuous and bijective and it maps the compact set $\tilde{\overline{\Rr}}_M$ onto the Hausdorff space $\overline{\Tt}_M$, so it is a homeomorphism. Consequently, $\tilde{\overline{\Rr}}_M$ is homeomorphic to a disc and its boundary is $\pi_M(\{\rho=M\}\cup\{\theta=0\}\cup\{\theta=\tfrac{\pi}{2}\})$, as required.
\qed
\end{Proof}
