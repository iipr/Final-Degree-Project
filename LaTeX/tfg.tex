...
\newpage	
\thispagestyle{empty}
\begin{abstract} In this work we gather three different proofs appearing in \cite{fg}, \cite{fgu} and \cite{fu1} of the following result: let $R$ be a real closed field and consider the open quadrant $\Qu:=\{x>0,y>0\}\subset R^2$. Then, there exists a polynomial map $f:R^2\to R^2$ such that $f(R^2)=\Qu$. In addition, we compare in the last chapter the computational complexity of the three maps $f$ satisfying the equality $f(R^2)=\Qu$ looking for the most efficient one. 

The first proof, which uses some basics in real algebraic geometry such as Sturm's theorem and the Curve selection lemma, requires the aid of a computer to determine the sign of some univariate polynomials in small intervals of the type $(0,\eps)$.  The second one has a purely algebraic character and to check that the image of $R^2$ is $\Qu$ is rather easy. However its execution time is high and the image of suitable grids is not thick enough. The third proof involves more sophisticated mathematical arguments, most of them concerning algebraic topology, but it seems to be very efficient from the computational view point.

As announced above we lastly perform a computational comparison in chapter 5, using SageMath, between the three polynomial maps that are used in the previous chapters, on the search for the most efficient and the quickest one.
...
En este trabajo reunimos tres demostraciones distintas, que aparecen en \cite{fg}, \cite{fgu} y \cite{fu1}, del siguiente resultado: dado un cuerpo real cerrado $R$ el cuadrante abierto $\Qu:=\{x>0,y>0\}\subset R^2$ es la imagen de una aplicaci\'on polin\'omica $f:R^2\to R^2$. Adem\'as comparamos en el \'ultimo cap\'itulo la complejidad computacional de las tres aplicaciones cuya imagen es el cuadrante abierto, tratando de encontrar la m\'as eficiente.

La primera demostraci\'on, que usa resultados elementales de la geometr\'ia algebraica real tales como el teorema de Sturm o el Lema de selecci\'on de curvas, requiere de la ayuda de un ordenador para estudiar el signo de ciertos polinomios en una variable en intervalos muy peque\~nos del tipo $(0,\eps)$. La segunda tiene car\'acter algebraico y la comprobaci\'on de que la imagen es el cuadrante abierto es muy sencilla. Sin embargo su coste en tiempo de ejecuci\'on es alto y el tamizado de la imagen de un cuadrado centrado en el origen de $R^2$ no es tan denso como en los restantes casos. Por \'ultimo la tercera prueba, que es la que usa argumentos matem\'aticos m\'as sofisticados, entre ellos algunos propios de la topolog\'ia algebraica, parece ser muy eficiente desde el punto de vista computacional.

La comparaci\'on de las complejidades a las que hemos hecho referencia en el p\'arrafo anterior se realiza mediante SageMath, es el contenido del cap\'itulo 5 y tiene por objetivo determinar cual de las tres aplicaciones es la m\'as eficiente y la m\'as r\'apida en rellenar el cuadrante.
...
\begin{theorem}[\em Tarski-Seidenberg\em]\label{tarskiSeidenberg} \em The image of every polynomial map $f:R^m\to R^n$ is a \hyperref[semialgSet]{semialgebraic subset} of $R^n$. \em
\end{theorem}
...
\end{axis}
\end{tikzpicture}
\caption[The homeomorphism $\zeta$ for $\xi_1(x,y):=\sqrt{B^2-\min(y^2,B^2)}$ acting on $\R^3$.]{The homeomorphism $\zeta$ for $\xi_1(x,y):=\sqrt{B^2-\min(y^2,B^2)}$ acting on $\R^3$. Notice how it ``flattens'' the warped disc $\D_1$.}\label{fig:homeo}
\end{center}
\end{figure}
%\end{comment}
...
Consider the canonical projection $\pi_M:\overline{\Rr}_M\to\tilde{\overline{\Rr}}_M$, which is continuous. Since $\phi_M$ is compatible with $\pi_M$, there exists a continuous map $\tilde{\phi}_M:\tilde{\Rr}_M\to\R^3$ such that the following diagram is commutative:
...
\end{tikzpicture}
\end{center}
\caption{Behavior of the map $\mu:\overline{\Rr}_M\to\overline{\Tt}_M$.}\label{fig:Tm}
\end{figure}
...
\end{tabular}}
\caption{The first polynomial map expanded.}\label{tab:firstMap}
\end{table}
\end{subsection}
...
\bibitem[A]{a} F. S. Acton: Numerical Methods That Work, 2\textsuperscript{nd} printing. Washington, DC: Math. Assoc. Amer., (1990). 

\bibitem[ABR]{abr} C. Andradas, L. Br\"ocker, J.M. Ruiz: Constructible sets in real geometry. \em Ergeb. Math. \em{\bf 33}. Berlin, Heidelberg, New York: Springer Verlag, (1996).
...
\bibitem[FU]{fu} J.F. Fernando, C. Ueno: A short proof for the open quadrant problem. \textit{Preprint RAAG}. Submitted to MEGA 2015, 8 pages.
...
\bibitem[P]{p} S. Pinchuk: A counterexample to the real Jacobian Conjecture, \em Math. Z. \em {\bf 217} (1994), 1--4.
...
\bibitem[W]{w} R.J. Walker: Algebraic curves. Berlin, Heidelberg, New York: Springer Verlag, (1978).
...
\begin{center}
\pythonexternal{../Laguerre/polyExample.py}	
\end{center}
