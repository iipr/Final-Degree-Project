...
We will start with some innocent grids of integers contained in the square $[-10, 10]\times[-10, 10]$ and in $[-100, 100]\times[-100, 100]$, and some dense sets in $[-1, 1]\times[-1, 1]$ and $[0, 1]\times[0, 1]$. Later, we will move into more specific sets provided by certain curves, following the ideas of  Section \ref{quadReasons}. Lastly, we will use random sets of points contained in the unit disc and in the disc of radius $\rho=100$.
...
The test we performed related to this were:
\begin{enumerate}[1. ]
...
$\PP_3$ & 410.73 s & 389.74 s & 155.09 s \\ \hline
$\PP_4$ & 408.88 s & 392.34 s & 155.29 s \\ \hline
$\PP_5$ & 17.10 s & 15.45 s & 7.00 s \\ \hline
$\PP_6$ & 1633.50 s & 1640.01 s & 625.11 s \\
\end{tabular}
...
$$
It took 149.99 s for $f_2$ to compute the image of the $10^6$ points from $\PP_7$, which is very close to the 155.09 s and 155.29 s that it took $\FFF$ to compute the image of the $10^6$ points from $\PP_3$ and $\PP_4$ respectively. % comparar con las imagenes de F(P_3) y F(P_4)
\end{subsection}
...
\end{tabular}
\caption[Times of the computation of points contained in ${\mathbb D}_{1}$ and ${\mathbb D}_{100}$.]{Times of the computation of ramdomly distibuted points in ${\mathbb D}_{1}$ and ${\mathbb D}_{100}$.}\label{tab:square}
\end{center}
\end{table}
...
\end{tabular}
\caption{Properties from the new maps.}\label{tab:newDegreeMon}
\end{center}
\end{table}
...
Note how good $\mathcal{N}_1$ seems to be on approaching the positive half-axes for this family of curves. Even when checking on $[0, 0.1]\times[0, 0.1]$ it fills plenty of area towards the axes.
%% REFERENCIA IMAGEN
...
\bibitem[FGU1]{fgu1} J.F. Fernando, C. Ueno: On the open quadrant as a polynomial image of $\R^2$ (revisited).
