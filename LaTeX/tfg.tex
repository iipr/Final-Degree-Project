...
\HH: \R^2\longrightarrow \R^2,\ (x, y)& \longmapsto (x(xy - 2)^2 + \sfrac{1}{2}\, xy^2,\ y). \\
\end{aligned}
}
\end{equation*}
...
Now we cand focus on proving $\mathscr{B} \subset \GG(\mathscr{A})$. First of all, notice that we can express the set $\mathscr{B}$ in the following way:
$$
\mathscr{B} = \bigsqcup_{x>0}(\set{x} \times [y_x, +\infty)) := \bigsqcup_{x>0}(\set{x} \times \mathscr{B}_x),
$$
where $y_x := \text{min}\set{x, 1/x}$. Intuitively, we are ``slicing'' the set $\mathscr{B}$ vertically, depending on $x$. Now look at the definition of $\GG_2$ and consider for each $x > 0$ the polynomial map $\phi$ dependent on the variable ${\tt y}$:
$$
\phi_x({\tt y}) := {\tt y}(x{\tt y} - 2)^2 + x (x{\tt y} - 1)^2 = x^2{\tt y}^3 + (x^3 - 4x){\tt y}^2 + (4 - 2x^2){\tt y} + x.
$$
The polynomials $\phi_x({\tt y})$ have odd degree and positive leading coefficient since $x > 0$. Now, notice that
\begin{equation*}
\begin{aligned}
\phi_x\left(\frac{1}{x}\right)& = \frac{1}{x} + \left((x^3 - 4x)\frac{1}{x^2} + (4 - 2x^2)\frac{1}{x} + x\right) = \frac{1}{x}\\
\phi_x\left(\frac{2}{x}\right)& = \left(\frac{8}{x} + (x^3 - 4x)\frac{4}{x^2} + (4 - 2x^2)\frac{2}{x}\right) + x = x.\\
\end{aligned}
\end{equation*}
So if we fix $x_0$ and taking into account $[1/x_0, +\infty) \supset [2/x_0, +\infty)$ we get that $\phi_x([1/x_0, +\infty)) \supset \phi_x([2/x_0, +\infty))$. Since the leading coeefficient of $\phi_x$ is positive we obtain that $\phi_x([1/x_0, +\infty)) = [1/x_0, +\infty)$ and $\phi_x([2/x_0, +\infty)) = [x_0, +\infty)$. So now it is clear that:
$$
...
The polynomial map 
$$
\HH := (\HH_1, \HH_2): \R^2 \longrightarrow \R^2, (x, y) \longmapsto (x(xy - 2)^2 + \sfrac{1}{2}\, xy^2,\ y)
$$
