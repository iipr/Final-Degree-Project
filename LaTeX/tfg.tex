...
\begin{subsection}{Getting ready}
...
&f_2:\R^2\to\R^2,\quad (x,y)\mapsto((xy^2+x^2y-y-1)^2+x^3y^2,(x^3y+xy-x-1)^2+x^3y^2)u
\end{align*}
As $f_1(\R^2)$ is the closed quadrant $\overline{\Qu}:=\{x\geq0,y\geq0\}$, we have to prove the equality
\begin{equation}
f_2(\overline{\Qu})=\Qu.
\end{equation}
The inclusion $f_2(\overline{\Qu})\subset\Qu$ is straightforward because both components of $f_2$ are strictly positive on $\overline{\Qu}$. It only remains to show the inclusion 
\begin{equation}\label{principal}
\Qu\subset f_2(\overline{\Qu}).
\end{equation}

Consider the (continuous) semialgebraic maps 
\begin{align*}
&g:\overline{\Qu}\rightarrow\R^3,\ (x,y)\mapsto(xy^2+x^2y-y-1,x^{3/2}y,x^3y+xy-x-1)\\
&h:\R^3\rightarrow\R^2,\ (x,y,z)\mapsto(x^2+y^2,y^2+z^2). 
\end{align*}
As $f_2=h\circ g$, we have to show that for each tuple $(A^2,B^2)\in\Qu$ there exists $(x_0,y_0)\in\overline{\Qu}$ such that $(h\circ g)(x_0,y_0)=(A^2,B^2)$. This is equivalent to check that the intersection $h^{-1}(\{(A^2,B^2)\})\cap g(\overline{\Qu})$ is non-empty. 

Denote $\mathcal{S}:=g(\overline{\Qu})$ and fix values $B\ge A>0$. It holds that sets
\begin{align*}
...
\begin{align}
\D_1:\,z=\xi_1(x,y),\quad x^2+y^2\le A^2,\label{d1}\\
\D_2:\,x=\xi_2(y,z),\quad y^2+z^2\le A^2,\label{d2}
\end{align}
...
\paragraph{}\hspace{-6mm}\label{311} {\em For fixed values $B\ge A>0$ the intersections $\partial\D_1\cap\mathcal{S}$ and $\partial\D_2\cap\mathcal{S}$ are non-empty.}
\end{subsection}

\begin{subsection}{Proof of Statement}
